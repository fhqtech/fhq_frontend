                      ...
                      {availablePrimaryInterviews.map((interview, index) => {
                        ...
                        return (
                          ...
                          >
                            <div className="flex flex-col h-full">
                              <div className="mb-3">
                                <h4 className={`font-medium text-sm transition-colors ${!isSelected ? 'group-hover:text-white' : ''}`}>{interview.title}</h4>
                                <p className={`text-[10px] text-gray-500 uppercase tracking-wider mt-1 transition-colors ${!isSelected ? 'group-hover:text-white/60' : ''}`}>
                                  #{interview.id}
                                </p>
                              </div>
                                ...
                                <div className="flex items-center justify-between">
                                  <span className={`text-[10px] px-2 py-0.5 rounded uppercase font-bold transition-colors ${
                                    interview.status === 'active'
                                      ? `bg-green-100 text-green-700 ${!isSelected ? 'group-hover:bg-green-600 group-hover:text-white' : ''}`
                                      : interview.status === 'completed'
                                      ? `bg-blue-100 text-blue-700 ${!isSelected ? 'group-hover:bg-blue-600 group-hover:text-white' : ''}`
                                      : `bg-gray-100 text-gray-700 ${!isSelected ? 'group-hover:bg-gray-600 group-hover:text-white' : ''}`
                                  }`}>
                                    {interview.status}
                                  </span>
                                </div>
