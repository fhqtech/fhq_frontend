...
function recommendationClasses(rec?: string): string {
  ...
  if (norm.includes('strong') && norm.includes('recommend')) return 'bg-emerald-50 text-emerald-700 border-emerald-200';
  if (norm.startsWith('recommend')) return 'bg-emerald-50 text-emerald-700 border-emerald-200';
  if (norm.includes('reservation')) return 'bg-amber-50 text-amber-800 border-amber-200';
  if (norm.includes('not recommend')) return 'bg-red-50 text-red-700 border-red-200';
      ...
      <main className="max-w-5xl mx-auto px-6 py-10 space-y-6">
        ...
        {nodes.length > 0 && (
          <div className="bg-white rounded-xl border border-border p-5">
            ...
            <TalentAnalysisGraph roleTitle={undefined} nodes={nodes} />
          </div>
        )}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div className="bg-white rounded-xl border border-border p-5">
            <p className="text-[10px] uppercase tracking-widest text-emerald-700 font-semibold mb-2">
              Strengths
            </p>
          ...
          <div className="bg-white rounded-xl border border-border p-5">
            <p className="text-[10px] uppercase tracking-widest text-red-700 font-semibold mb-2">
              Areas to develop
            </p>
            {(r.gaps || []).length === 0 ? (
              <p className="text-sm text-foreground-muted">No gaps recorded.</p>
            ) : (
              <ul className="space-y-2 text-sm text-foreground/85 list-disc pl-5">
                {(r.gaps || []).map((g, i) => (
