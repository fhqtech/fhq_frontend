...
export default function CandidateRegistration() {
  ...
  const handleNext = () => {
    ...
    // Validate psychAssessment animal (required)
    if (!formData.psychAssessment.animal) {
      errors.psychAnimal = "Please select at least the animal question";
      console.log("❌ Validation failed: Animal question is missing");
    }
            ...
            <CardContent className="p-8 md:p-10">
              ...
              <div className="mt-8">
                <div className="bg-slate-50 rounded-2xl p-4">
                  <div className="mb-3">
                    <div className="flex items-center gap-2">
                      <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="#000000" viewBox="0 0 256 256"><path d="M160,40a32,32,0,1,0-32,32A32,32,0,0,0,160,40ZM128,56a16,16,0,1,1,16-16A16,16,0,0,1,128,56ZM231.5,87.71A19.62,19.62,0,0,0,212,72H44a20,20,0,0,0-8.38,38.16l.13,0,50.75,22.35-21,79.72A20,20,0,0,0,102,228.8l26-44.87,26,44.87a20,20,0,0,0,36.4-16.52l-21-79.72,50.75-22.35.13,0A19.64,19.64,0,0,0,231.5,87.71Zm-17.8,7.9-56.93,25.06a8,8,0,0,0-4.51,9.36L175.13,217a7,7,0,0,0,.49,1.35,4,4,0,0,1-5,5.45,4,4,0,0,1-2.25-2.07,6.31,6.31,0,0,0-.34-.63L134.92,164a8,8,0,0,0-13.84,0L88,221.05a6.31,6.31,0,0,0-.34.63,4,4,0,0,1-2.25,2.07,4,4,0,0,1-5-5.45,7,7,0,0,0,.49-1.35L103.74,130a8,8,0,0,0-4.51-9.36L42.3,95.61A4,4,0,0,1,44,88H212a4,4,0,0,1,1.73,7.61Z"></path></svg>
                      <h3 className="font-medium text-base text-slate-900 uppercase tracking-wider">Your Vibe</h3>
                    </div>
                  </div>
                  ...
                  <div className="space-y-3">
                    {/* Animal Question */}
                    <div>
                      <Label className="uppercase text-xs tracking-wider mb-2 block">{PSYCH_QUESTIONS.animal.question} <span className="text-red-600">*</span></Label>
                      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                        {PSYCH_QUESTIONS.animal.options.map((option) => (
                          <button
                            ...
                            className={`pl-6 pr-4 py-4 rounded transition-all flex items-center gap-4 relative ${
                            ...
                            style={{
                              boxShadow: formData.psychAssessment.animal === option.value ? '0 4px 12px rgba(0,0,0,0.15)' : 'inset 1px 1px 2px #e8e8e8, 2px 2px 4px #d5d5d5'
                            }}
                          >
                            <div className="text-3xl">{option.emoji}</div>
                            <div className="font-medium text-base text-slate-900 uppercase tracking-wide">{option.label}</div>
                            {formData.psychAssessment.animal === option.value && (
                      ...
                      {formErrors.psychAnimal && (
                        <p className="text-xs text-red-500 mt-2">{formErrors.psychAnimal}</p>
                      )}
                    </div>
                    ...
                    <div className="pt-3">
                      <button
                        type="button"
                        onClick={() => setShowBonusQuestions(!showBonusQuestions)}
                        className="text-sm text-slate-600 flex items-center gap-2 hover:text-slate-900 transition-colors"
                      >
                        <Sparkles className="w-4 h-4" />
                        More personality questions (optional)
                        <ChevronRight className={`w-4 h-4 transition-transform ${showBonusQuestions ? 'rotate-90' : ''}`} />
                      </button>
                    </div>
                    ...
                    {showBonusQuestions && (
                      <div className="space-y-3 pt-3">
                        ...
                        <div>
                          <Label className="uppercase text-xs tracking-wider mb-2 block">{PSYCH_QUESTIONS.color.question} <span className="text-slate-500 text-xs">(Optional)</span></Label>
                          <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                            {PSYCH_QUESTIONS.color.options.map((option) => (
                              <button
                                ...
                                style={{
                                  boxShadow: formData.psychAssessment.color === option.value ? '0 4px 12px rgba(0,0,0,0.15)' : 'inset 1px 1px 2px #e8e8e8, 2px 2px 4px #d5d5d5'
                                }}
                              >
                                <div className="text-3xl">{option.emoji}</div>
                                <div className="font-medium text-base text-slate-900 uppercase tracking-wide">{option.label}</div>
                        ...
                        <div>
                          <Label className="uppercase text-xs tracking-wider mb-2 block">{PSYCH_QUESTIONS.environment.question} <span className="text-slate-500 text-xs">(Optional)</span></Label>
                          <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                            {PSYCH_QUESTIONS.environment.options.map((option) => (
                              <button
                                ...
                                style={{
                                  boxShadow: formData.psychAssessment.environment === option.value ? '0 4px 12px rgba(0,0,0,0.15)' : 'inset 1px 1px 2px #e8e8e8, 2px 2px 4px #d5d5d5'
                                }}
                              >
                                <div className="text-3xl">{option.emoji}</div>
                                <div className="font-medium text-base text-slate-900 uppercase tracking-wide">{option.label}</div>
                        ...
                        <div>
                          <Label className="uppercase text-xs tracking-wider mb-2 block">{PSYCH_QUESTIONS.symbol.question} <span className="text-slate-500 text-xs">(Optional)</span></Label>
                          <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                            {PSYCH_QUESTIONS.symbol.options.map((option) => (
                              <button
                                ...
                                style={{
                                  boxShadow: formData.psychAssessment.symbol === option.value ? '0 4px 12px rgba(0,0,0,0.15)' : 'inset 1px 1px 2px #e8e8e8, 2px 2px 4px #d5d5d5'
                                }}
                              >
                                <div className="text-3xl">{option.emoji}</div>
                                <div className="font-medium text-base text-slate-900 uppercase tracking-wide">{option.label}</div>
