...
export default function ManageInterviewsEnhanced() {
 ...
 // Interviews list — TanStack Query owns fetch state. SSE bumps
 // `liveRevision` ~5s after backend changes; the bridge invalidates
 // this cache so the table reflects candidate progress without manual
 // wiring. The empty-state JSX below already guards on `isLoading` so
 // it doesn't flash before workspace+project resolve.
 const interviewsQuery = useInterviewsQuery(currentWorkspace?.id, currentProject?.id);
 const interviews: Interview[] = interviewsQuery.data?.interviews ?? [];
 const isLoading = interviewsQuery.isPending && Boolean(currentWorkspace && currentProject);
 ...
 // project's interview list. The bridge invalidates the interviews
 // query so subscribers refetch automatically.
 ...
 try {
 await deleteInterview.mutateAsync(interviewToDelete.id);
 toast({
 ...
 toast({
 title: "Delete Failed",
 description: err instanceof Error ? err.message : "Failed to delete the interview. Please try again.",
 variant: "destructive"
 });
 } finally {
 ...
 const data = await startInterview.mutateAsync(interviewId);
 ...
 if (rate > 0) return "text-danger cursor-pointer hover:underline";
 return "text-muted";
 };
 ...
 <div className="rounded-sm p-6 bg-paper shadow-[0_2px_8px_rgba(0,0,0,0.08)] transition-shadow duration-200">
 ...
 {/* Interviews Table */}
 <div
 className="rounded-sm bg-paper border border-border"
 style={{
 boxShadow: 'var(--shadow-clay)'
 }}
 >
 ...
 <EmptyState
 icon={Users}
 title={searchTerm ? "No matching interviews" : "No interviews yet"}
 description={
 searchTerm
 ? "Try adjusting your search to find more interviews."
 : "Get started by creating your first AI interview. Set up voice settings, add applicants, and begin screening efficiently."
 }
 primaryAction={
 searchTerm
 ? { label: "Clear search", variant: "outline", onClick: () => setSearchTerm("") }
 : { label: "Create your first interview", onClick: () => navigate(`/interviews/create?type=${interviewType}`) }
 }
 />
 ) : (
 ...
 <Users weight="fill" className="w-3 h-3 text-muted" />
 ...
 <TableCell className="w-[60px] hidden md:table-cell text-muted text-sm text-center">
 {interview.duration}
 </TableCell>
 <TableCell className="w-[100px] hidden lg:table-cell text-muted text-xs whitespace-nowrap">
