...
// Proficiency level config (matching SkillsGraph)
const proficiencyConfig: Record<number, { short: string; full: string; color: string }> = {
  1: { short: 'NOOB', full: 'Beginner', color: '#14b8a6' },
  2: { short: 'BASIC', full: 'Basic', color: '#3b82f6' },
  3: { short: 'MID', full: 'Intermediate', color: '#8b5cf6' },
  4: { short: 'PRO', full: 'Advanced', color: '#ec4899' },
  5: { short: 'GOAT', full: 'Expert', color: '#ef4444' }
};
    ...
    <div className="relative bg-black py-12">
      <div className="max-w-7xl mx-auto px-6">
        ...
        <div className="text-center mb-8">
          <h2 className="text-4xl md:text-5xl font-light text-white tracking-wide mb-3 transition-opacity duration-300">
            {currentSlide === 0 ? (
              <>How Flowy <span className="text-transparent bg-clip-text bg-gradient-to-r from-purple-400 to-pink-400">Evaluates</span></>
            ) : (
              <>Flowy Can Be <span className="text-transparent bg-clip-text bg-gradient-to-r from-purple-400 to-pink-400">Configured</span></>
            )}
          </h2>
          <p className="text-white/50 text-lg uppercase tracking-wide">
            {currentSlide === 0
        ...
        <div className="relative">
          <div className="absolute -inset-1 bg-gradient-to-r from-purple-500/20 via-pink-500/20 to-purple-500/20 rounded-2xl blur-xl opacity-50" />

          {/* Left Arrow - Only visible on slide 2 */}
          {currentSlide === 1 && (
            <button
              onClick={() => setCurrentSlide(0)}
              className="absolute left-0 top-1/2 -translate-y-1/2 -translate-x-1/2 z-10 p-3 rounded-full bg-white/10 border border-white/20 backdrop-blur-sm transition-all hover:bg-white/20"
            >
          ...
          {currentSlide === 0 && (
            <button
              ...
              className="absolute right-0 top-1/2 -translate-y-1/2 translate-x-1/2 z-10 p-3 rounded-full bg-white/10 border border-white/20 backdrop-blur-sm transition-all hover:bg-white/20"
            >
              <ChevronRight className="w-6 h-6 text-white" />
            </button>
          )}

          <div className="relative bg-gradient-to-b from-white/[0.08] to-white/[0.02] border border-white/10 rounded-2xl overflow-hidden">
            {/* Slides Container */}
                  ...
                  <div className="px-6 py-4 border-b border-white/10 bg-white/[0.02]">
                    <div className="flex flex-wrap items-center justify-between gap-4">
                      <div className="flex items-center gap-3">
                        <h3 className="font-mono font-bold text-lg text-white uppercase tracking-wide">{editTitle}</h3>
                        <span className="text-[10px] font-mono font-bold px-2 py-1 rounded uppercase bg-purple-500/20 text-purple-300">
                          {editType}
                        </span>
                        <span className="bg-white/10 text-white/70 text-[10px] font-mono font-bold px-2 py-1 rounded uppercase">
                          skill tree
                        </span>
                      </div>
                    ...
                    <p className="text-white/50 text-sm font-mono mt-2 max-w-3xl">{editTopics}</p>
                  </div>

                  <div className="flex flex-col lg:flex-row min-h-[420px]">
                    {/* Left Sidebar - Certs & Tools */}
                    <div className="w-full lg:w-56 flex-shrink-0 border-b lg:border-b-0 lg:border-r border-white/10 bg-white/[0.02] p-4 flex flex-row lg:flex-col gap-4">
                      <div className="flex-1 lg:flex-none bg-white/5 border border-white/10 rounded-lg p-3">
                        <div className="flex items-center gap-2 mb-3">
                          <Award className="w-4 h-4 text-amber-400" />
                          <p className="text-[10px] font-mono font-bold text-white/40 uppercase tracking-wider">certs</p>
                        </div>
                        <div className="flex flex-col gap-1.5">
                          {editCertifications.map((cert, i) => (
                            <div key={i} className="flex items-start gap-2">
                              <div className="w-1.5 h-1.5 rounded-full bg-amber-400 mt-1.5 flex-shrink-0" />
                              <span className="text-[10px] font-mono text-white/60 leading-tight">{cert}</span>
                            </div>
                          ))}
                        </div>
                      </div>
                      <div className="flex-1 lg:flex-none bg-white/5 border border-white/10 rounded-lg p-3">
                        <div className="flex items-center gap-2 mb-3">
                          ...
                          <p className="text-[10px] font-mono font-bold text-white/40 uppercase tracking-wider">tools</p>
                        </div>
                        <div className="flex flex-col gap-2">
                          {editTools.map((tool, i) => (
                            <div key={i} className="flex flex-col">
                              <span className="text-[10px] font-mono font-bold text-white/70">{tool.name}</span>
                              <span className="text-[9px] font-mono text-white/40">{tool.category}</span>
                            </div>
                          ))}
                        </div>
                      </div>
                    ...
                    <div className="w-full lg:w-64 flex-shrink-0 border-t lg:border-t-0 lg:border-l border-white/10 bg-white/[0.02] p-4 flex flex-col gap-3">
                      <div className="bg-white/5 border border-white/10 rounded-lg p-3">
                        <p className="text-[10px] font-mono font-bold text-white/40 uppercase tracking-wider mb-3">Proficiency Levels</p>
                        <div className="flex flex-col gap-1.5">
                          {[1, 2, 3, 4, 5].map((level) => (
                            <div key={level} className="flex items-center gap-2">
                              <div className="w-3 h-3 rounded-full flex-shrink-0" style={{ backgroundColor: proficiencyConfig[level].color }} />
                              <div className="px-2 py-0.5 rounded bg-white/10 border border-white/10 flex items-center justify-center min-w-[40px]">
                                <span className="text-[9px] font-mono font-bold text-white/70">{proficiencyConfig[level].short}</span>
                              </div>
                              <span className="text-[10px] font-mono text-white/50">{proficiencyConfig[level].full}</span>
                            </div>
                          ))}
                        </div>
                      </div>
                      {selectedSkill ? (
                        <div className="bg-white/10 border border-white/20 rounded-lg p-4 flex-1">
                          <div className="flex items-start justify-between mb-2">
                            <p className="font-mono font-bold text-sm text-white uppercase tracking-wide flex-1">{selectedSkill.name}</p>
                            <button onClick={() => setSelectedSkill(null)} className="bg-white/10 hover:bg-white/20 text-white/60 p-1 rounded transition-colors ml-2">
                              <X className="w-3 h-3" />
                            </button>
                          </div>
                          <p className="text-white/50 text-xs font-mono leading-relaxed">{selectedSkill.description}</p>
                          <div className="mt-3 pt-3 border-t border-white/10">
                            <div className="flex items-center gap-2">
                              <span className="text-white/40 text-[10px] font-mono">Required Level:</span>
                              <span className="px-2 py-0.5 text-white rounded font-bold text-[10px] font-mono" style={{ backgroundColor: proficiencyConfig[selectedSkill.expected_proficiency]?.color }}>
                        ...
                        <div className="bg-white/5 border-2 border-dashed border-white/10 rounded-lg px-4 py-6 text-center flex-1 flex items-center justify-center">
                          <p className="text-xs text-white/30 font-mono">Click a skill to see details</p>
                        </div>
                      )}
                    </div>
                  </div>
                  ...
                  <div className="flex flex-col lg:flex-row min-h-[450px]">
                    ...
                    <div className="w-full lg:w-2/5 p-6 border-b lg:border-b-0 lg:border-r border-white/10 overflow-y-auto flex flex-col">
                      <div className="space-y-4 flex-1 flex flex-col">
                        <div>
                          <label className="block text-xs font-bold text-white/50 uppercase mb-2">Blueprint Title / Target Role</label>
                          <input
                            type="text"
                            value={editTitle}
                            onChange={e => setEditTitle(e.target.value)}
                            className="w-full px-4 py-3 bg-white/5 border border-white/20 rounded-lg text-white text-sm outline-none focus:ring-2 focus:ring-purple-500 focus:border-transparent transition-all"
                            placeholder="e.g. Senior Frontend Developer"
                          />
                        </div>
                        <div className="flex-1 flex flex-col">
                          <label className="block text-xs font-bold text-white/50 uppercase mb-2">Interview Topics / Description</label>
                          <textarea
                            value={editTopics}
                            onChange={e => setEditTopics(e.target.value)}
                            className="w-full flex-1 min-h-[120px] px-4 py-3 bg-white/5 border border-white/20 rounded-lg text-white text-sm outline-none focus:ring-2 focus:ring-purple-500 focus:border-transparent resize-none transition-all"
                        ...
                        <div className="grid grid-cols-2 gap-4">
                          <div>
                            ...
                            <select
                              ...
                              className="w-full px-4 py-2.5 bg-white/5 border border-white/20 rounded-lg text-white text-sm outline-none focus:ring-2 focus:ring-purple-500"
                            >
                          ...
                          <div>
                            ...
                            <div className="flex bg-white/5 rounded-lg p-1 border border-white/10">
                              <button
                                ...
                                className={`flex-1 px-3 py-1.5 rounded-md text-xs font-medium transition-colors ${editType === 'screening' ? 'bg-emerald-500/30 text-emerald-300' : 'text-white/60 hover:text-white'}`}
                              >
                                Screening
                              </button>
                              <button
                                onClick={() => setEditType('fitment')}
                                className={`flex-1 px-3 py-1.5 rounded-md text-xs font-medium transition-colors ${editType === 'fitment' ? 'bg-blue-500/30 text-blue-300' : 'text-white/60 hover:text-white'}`}
                              >
                                Fitment
                              </button>
                            </div>
                        ...
                        <div>
                          <label className="block text-xs font-bold text-white/50 uppercase mb-2">Visibility</label>
                          <div className="flex bg-white/5 rounded-lg p-1 border border-white/10">
                            <button
                              onClick={() => setEditScope('global')}
                              className={`flex-1 px-4 py-2 rounded-md text-xs font-medium transition-colors flex items-center justify-center gap-2 ${editScope === 'global' ? 'bg-indigo-500/30 text-indigo-300' : 'text-white/60 hover:text-white'}`}
                            >
                              <Globe className="w-3 h-3" /> Global
                            </button>
                            <button
                              onClick={() => setEditScope('private')}
                              className={`flex-1 px-4 py-2 rounded-md text-xs font-medium transition-colors flex items-center justify-center gap-2 ${editScope === 'private' ? 'bg-amber-500/30 text-amber-300' : 'text-white/60 hover:text-white'}`}
                            >
                    ...
                    <div className="w-full lg:w-3/5 p-4 bg-white/[0.02] overflow-y-auto flex flex-col">
                      <div className="flex-1 flex flex-col gap-3">
                        {/* Skills Section - Takes most space */}
                        <div className="bg-white/5 border border-white/10 rounded-lg p-4 flex-1 flex flex-col min-h-0">
                          ...
                          <div className="flex gap-2 mb-3">
                            <input
                              ...
                              className="flex-1 px-3 py-1.5 bg-white/5 border border-white/20 rounded-lg text-white text-xs outline-none focus:ring-2 focus:ring-purple-500"
                              onKeyDown={(e) => e.key === 'Enter' && handleAddSkill()}
                            />
                            <select
                              value={newSkillProficiency}
                              onChange={(e) => setNewSkillProficiency(Number(e.target.value))}
                              className="px-3 py-1.5 bg-gray-800 border border-white/20 rounded-lg text-white text-xs outline-none focus:ring-2 focus:ring-purple-500"
                              style={{ minWidth: '60px' }}
                            >
                              {[1, 2, 3, 4, 5].map(l => (
                                <option key={l} value={l} className="bg-gray-900 text-white">L{l}</option>
                              ))}
                            </select>
                            <button onClick={handleAddSkill} disabled={!newSkillName.trim()} className="px-3 py-1.5 bg-purple-500 text-white rounded-lg text-xs disabled:opacity-50 hover:bg-purple-600 transition-colors">
                              <Plus className="w-3 h-3" />
                            </button>
                          </div>
                          <div className="flex-1 overflow-y-auto space-y-1.5">
                            {editSkills.map((skill) => (
                              <div key={skill.skill_id} className="flex items-center justify-between p-2 bg-white/5 rounded-lg group">
                                <span className="text-xs text-white truncate flex-1 mr-2">{skill.name}</span>
                                <div className="flex items-center gap-2">
                                  <select
                                  ...
                                  <button onClick={() => handleRemoveSkill(skill.skill_id)} className="p-0.5 text-white/30 hover:text-red-400 opacity-0 group-hover:opacity-100 transition-all">
                                    <X className="w-3 h-3" />
                                  </button>
                                </div>
                              </div>
                        ...
                        <div className="grid grid-cols-2 gap-3">
                          {/* Tools Section */}
                          <div className="bg-white/5 border border-white/10 rounded-lg p-3">
                            <h4 className="text-xs font-bold text-white/50 uppercase mb-2 flex items-center gap-2">
                              <Wrench className="w-3 h-3 text-blue-400" />
                              Tools ({editTools.length})
                            </h4>
                            <div className="flex gap-2 mb-2">
                              <input type="text" value={newToolName} onChange={(e) => setNewToolName(e.target.value)} placeholder="Tool name..." className="flex-1 px-2 py-1 bg-white/5 border border-white/20 rounded text-white text-[10px] outline-none focus:ring-1 focus:ring-blue-500" onKeyDown={(e) => e.key === 'Enter' && handleAddTool()} />
                              <button onClick={handleAddTool} disabled={!newToolName.trim()} className="px-2 py-1 bg-blue-500 text-white rounded text-[10px] disabled:opacity-50 hover:bg-blue-600 transition-colors">
                                <Plus className="w-3 h-3" />
                              </button>
                            </div>
                          ...
                          <div className="bg-white/5 border border-white/10 rounded-lg p-3">
                            ...
                            <div className="flex gap-2 mb-2">
                              <input type="text" value={newCertification} onChange={(e) => setNewCertification(e.target.value)} placeholder="Add cert..." className="flex-1 px-2 py-1 bg-white/5 border border-white/20 rounded text-white text-[10px] outline-none focus:ring-1 focus:ring-amber-500" onKeyDown={(e) => e.key === 'Enter' && handleAddCertification()} />
                              <button onClick={handleAddCertification} disabled={!newCertification.trim()} className="px-2 py-1 bg-amber-500 text-white rounded text-[10px] disabled:opacity-50 hover:bg-amber-600 transition-colors">
                                <Plus className="w-3 h-3" />
                              </button>
                            </div>
                              ...
                              {editCertifications.map((cert, index) => (
                                <div key={index} className="flex items-center justify-between group">
                                  <div className="flex items-start gap-1.5">
                                    ...
                                    <span className="text-[9px] text-white/60 leading-tight">{cert}</span>
                                  </div>
                                  <button onClick={() => handleRemoveCertification(index)} className="p-0.5 text-white/30 hover:text-red-400 opacity-0 group-hover:opacity-100 transition-all">
                                    <X className="w-2 h-2" />
                                  </button>
                                </div>
                              ))}
            ...
            {/* Dot Indicators */}
            <div className="flex items-center justify-center gap-3 py-4 border-t border-white/10 bg-white/[0.02]">
              <button
                onClick={() => setCurrentSlide(0)}
                className={`w-2.5 h-2.5 rounded-full transition-all duration-300 ${currentSlide === 0 ? 'bg-purple-500 w-8' : 'bg-white/20 hover:bg-white/40'}`}
              />
              <button
                onClick={() => setCurrentSlide(1)}
                className={`w-2.5 h-2.5 rounded-full transition-all duration-300 ${currentSlide === 1 ? 'bg-purple-500 w-8' : 'bg-white/20 hover:bg-white/40'}`}
              />
            </div>
          </div>
        </div>
...
const Navigation = () => {
  ...
  return (
    <nav className="fixed top-0 left-0 w-full z-20 flex justify-between items-center p-6 md:p-8 bg-black/80 backdrop-blur-sm">
      ...
      <div className="hidden md:flex space-x-8 text-sm font-medium text-white/70">
        ...
        <a href="/#meet-flowy" className="hover:text-white transition-colors">See Me In Action</a>
...
const HowItWorks = () => {
  return (
    <div className="relative w-full min-h-screen bg-black selection:bg-blue-500 selection:text-white">
      <Navigation />
